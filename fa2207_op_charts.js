...
d3.csv(csv_dir_url + "master_board.csv").then( d => {
  ...
  const fig7_3 = Plot.plot({
    ...
    marginRight: chart_options.marginRight,
    x:{label: "School Boards", nice: true, tickFormat: d => null},
    y:{domain: [-10, 90], label: "Accumulated Surplus / (Deficit) as a Share of Revenue", tickFormat: d => d + "%"},
    marks:[
      ...
      Plot.tip(fig7_3_data, Plot.pointerX({
        x: "Board",
        y: "Year-End Accumulated Surplus as Share of Revenue (August 31, 2022)",
        title: (d) => "School Board: " + `${d.Board}` + "\nSchool System: " + `${d.System},` + "\nAccumulated Surplus / (Deficit) as a Share of Revenue: " + `${(Math.round(+d["Year-End Accumulated Surplus as Share of Revenue (August 31, 2022)"]*10)/10).toFixed(1)}` + "%",
        lineWidth: 1000,
      }))
    ],
    color: {legend: true, domain: ["English Public", "English Catholic", "French Public", "French Catholic"], range: [fao_blue, fao_light_blue_1, fao_pink, fao_light_pink]}
  ...
  const fig8_4 = Plot.plot({
    ...
    marks:[
      ...
      Plot.tip(d, Plot.pointerX({
        x: "Board",
        y: "EQAO Pass Rate",
        title: (d) => "School Board: " + `${d.Board}` + "\nEQAO Pass Rate: " + `${(+d["EQAO Pass Rate"]*100).toFixed(1)}` + "%\nUrban Factor: " + `${d.Urban}`,
        lineWidth: 1000,
      }))
    ],
...
d3.csv(csv_dir_url + "fig6.5_data.csv").then(d => {
  ...
  const fig6_5 =
    Plot.plot({
      ...
      color: {legend: true, domain: ["Teacher Compensation", "Non-teacher Instruction", "Administration", "Transportation", "Pupil Accommodation", "Infrastructure", "Other Spending"], range: ["#255FD5", "#93BBF7", fao_pink,"#D4E3FC", "#1A2B4A", fao_dark_blue, fao_green]}
    })
